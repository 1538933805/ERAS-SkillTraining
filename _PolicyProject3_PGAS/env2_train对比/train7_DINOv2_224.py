...
ckpt_path =              (".\\_PolicyProject3_PGAS\\env2_train对比\\7_DINOv2_224\\model.ckpt")
output_dir =             (".\\_PolicyProject3_PGAS\\env2_train对比\\7_DINOv2_224")
training_loss_csv_path = (".\\_PolicyProject3_PGAS\\env2_train对比\\7_DINOv2_224\\loss.csv")
...
vision_encoder_1 = nets_VE1.DINOv2VisionEncoder(is_224=True)
vision_encoder_2 = nets_VE1.DINOv2VisionEncoder(is_224=True)
# "可选择冻结预训练权重"
                ...
                with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
                    for batch_idx, nbatch in enumerate(tepoch):
                        try:
                            ...
                            # device transfer
                            # processor(images=x, return_tensors="pt")
                            nimage_1 = nbatch['image_1'][:,:obs_horizon].to(device)
                            nimage_2 = nbatch['image_2'][:,:obs_horizon].to(device)
                            # print("mimage_1:\t", nimage_1)
                            # print("mimage_1形状:\t", nimage_1.shape)
                            # "改! DINO需要预处理图像!!!!!"
                            # # 合并批次和时间维度
                            # batch_size, T = nimage_1.shape[0], nimage_1.shape[1]
                            # nimage_1_flat = nimage_1.reshape(-1, *nimage_1.shape[2:])  # [2*obs_horizon, 3, 256, 256]
                            # nimage_2_flat = nimage_2.reshape(-1, *nimage_2.shape[2:])  # [2*obs_horizon, 3, 256, 256]
                            # # 预处理（假设 processor 支持批处理）
                            # processed_1 = processor(images=nimage_1_flat, return_tensors="pt")
                            # processed_2 = processor(images=nimage_2_flat, return_tensors="pt")
                            # # 提取张量并移到设备
                            # pixel_values_1 = processed_1['pixel_values'].to(device)  # [2*obs_horizon, C, H, W]
                            # pixel_values_2 = processed_2['pixel_values'].to(device)
                            # # 恢复原始维度
                            # nimage_1 = pixel_values_1.reshape(batch_size, obs_horizon, *pixel_values_1.shape[1:])  # [2, obs_horizon, C, H, W]
                            # nimage_2 = pixel_values_2.reshape(batch_size, obs_horizon, *pixel_values_2.shape[1:])  # [2, obs_horizon, C, H, W]
                            # "改! DINO需要预处理图像!!!!!"
                            # print("mimage_1形状:\t", nimage_1.shape)
