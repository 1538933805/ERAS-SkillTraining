...
training_loss_csv_path = (".\\_PolicyProject3_PGAS\\env2_train对比\\5_DINOv2_1\\补充.csv")
                ...
                with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
                    for batch_idx, nbatch in enumerate(tepoch):
                        try:
                            ...
                            # # 获取原始形状
                            # shape_1 = nbatch['image_1'][:, :obs_horizon].shape
                            # shape_2 = nbatch['image_2'][:, :obs_horizon].shape
                            # # 创建形状相同的随机像素张量 (值范围 [0, 1))
                            # nimage_1 = torch.rand(shape_1, device=device)
                            # nimage_2 = torch.rand(shape_2, device=device)
