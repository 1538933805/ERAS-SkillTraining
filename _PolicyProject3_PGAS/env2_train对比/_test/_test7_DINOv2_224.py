...
info_path = (".\\_PolicyProject3_PGAS\\env2_train对比\\_test\\7_DINOv2_224\\info.csv")
...
env = assemble_env_2.AssembleEnv(img_obs_height=img_height, img_obs_width=img_width, img_obs_channel=img_channel,
...
env.rob.RTDE_SOFT_F_THRESHOLD = 20
env.rob.RTDE_SOFT_RETURN = 0.005
...
for episode in range(20):
    ...
    env.rob.moveIK_changePosOrt_onEnd(d_pos=[0,0,-0.02], d_ort=[0,0,0], threshold_time=0.5, wait=True)
    env.rob.moveIK_changePosOrt_onEnd(d_pos=[0,0,-0.02], d_ort=[0,0,0], threshold_time=0.5, wait=True)
    ...
    for ps in range(pred_steps):
        ...
        # print("nimage_obses_1形状:\t", nimage_obses_1.shape)
        # "改! DINO需要预处理图像!!!!!"
        # nimage_obses_1 = processor(images=nimage_obses_1, return_tensors="pt")["pixel_values"].to(device, dtype=torch.float32)
        # nimage_obses_2 = processor(images=nimage_obses_2, return_tensors="pt")["pixel_values"].to(device, dtype=torch.float32)
        # # print("nimage_obses_1形状:\t", nimage_obses_1.shape)
        # "改! DINO需要预处理图像!!!!!"
