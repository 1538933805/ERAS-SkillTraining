...
ckpt_path =              (".\\_PolicyProject3_PGAS\\env1_train对比\\2_Transformer\\model.ckpt")
output_dir =             (".\\_PolicyProject3_PGAS\\env1_train对比\\2_Transformer")
training_loss_csv_path = (".\\_PolicyProject3_PGAS\\env1_train对比\\2_Transformer\\loss.csv")
batch_reduce = 8 # GPU不足时,batch_size缩小倍数
max_iterations = 50000 * batch_reduce  # 目标最大迭代次数
"记录标记!"
"记录标记!"
# 测试参数配置
test_result_path = "./_PolicyProject3_PGAS/env1_train对比/_网络性能/2_Transformer/network_performance.txt"  # 测试结果保存路径
...
def create_network():
    ...
    # 噪声预测网络
    obs_dim = 512 * obs_horizon  # 来自原始代码设置
...
def profile_gpu_memory(model, dataloader):
    ...
    obs_cond = torch.cat([feat1, feat2], dim=-1)
        
    # 噪声预测
    pred = model['noise_pred_net'](sample=noise, timestep=timesteps, cond=obs_cond)
