...
def apply_augmentation(image, params):
    ...
    # 色调调整
    hsv[:, :, 0] += params['hue']
    hsv[:, :, 0] = np.mod(hsv[:, :, 0], 180)  # 色调范围0-179
...
def main(dataset_path):
    ...
    # 加载元数据
    episode_ends = pd.read_csv(paths['episode_ends'], header=None)[0].tolist()
    ...
    current_max_idx = episode_ends[-1] if episode_ends else 0
    new_episode_ends = episode_ends.copy()

    # 遍历每个原始episode
    for ep_idx in tqdm(range(len(episode_ends)), desc="Processing episodes"):
        # 计算当前episode范围
        start = episode_ends[ep_idx-1] if ep_idx > 0 else 0
        end = episode_ends[ep_idx]
        episode_length = end - start

        # 生成9个增强版本
        for aug_idx in tqdm(range(9), desc=f"Augmenting episode {ep_idx+1}", leave=False):
            # 生成增强参数
            ...
            # 处理每个时间步
            for step in tqdm(range(int(episode_length)), desc="Processing steps", leave=False):
                original_idx = int(start + step)
                new_idx = int(current_max_idx + step)

                # 处理图像观测
                for cam in ['img_obs_1', 'img_obs_2']:
                    img_path = os.path.join(paths[cam], f"{cam}_{int(original_idx):012d}.png")
                    ...
                    # 保存新图像
                    new_img_path = os.path.join(paths[cam], f"{cam}_{int(new_idx):012d}.png")
