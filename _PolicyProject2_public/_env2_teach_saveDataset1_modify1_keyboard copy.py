...
dataset_zip_path = (".\\_PolicyProject2_public\\_trained_models_env2\\_teach1_dataset.zip")
dataset_hdf5_path = (".\\_PolicyProject2_public\\_trained_models_env2\\_teach1_dataset.hdf5")
"此齿轮装配设置多种初始位置误差域的选择,默认为1"
initial_option = 2
...
env = assemble_env_2.AssembleEnv()
...
if __name__ == '__main__':
    while True:
        ...
        if initial_option == 2:
            env.reset(isAddError=True, is_drag_mode=is_drag_mode,
                      initial_pos_ort=np.array([-5.2e-3, -612.5e-3, 92.5e-3, 0, -3.1415, 0]),
                      error_scale_pos=0.5, error_scale_ort=0.5)
        else:
            env.reset(isAddError=True, is_drag_mode=is_drag_mode)
