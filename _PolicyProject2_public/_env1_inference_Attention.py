...
dataset_path =           (".\\_PolicyProject2_public\\_trained_models_env1\\_teach1_dataset")
ckpt_path =              (".\\_PolicyProject2_public\\_trained_models_env1\\model_PolicyProject1_Attention.ckpt")
output_dir =             (".\\_PolicyProject2_public\\_trained_models_env1")
training_loss_csv_path = (".\\_PolicyProject2_public\\_trained_models_env1\\training_loss_Attention.csv")
MinMax_dir_path =        (".\\_PolicyProject2_public\\_trained_models_env1\\MinMax")
...
class PolicyProject1Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        action = action_csv.to_numpy().astype(np.float32).reshape(-1, 6)
...
action_horizon
#|o|o|                         observations: 2
#| |a|a|a|a|a|a|               actions executed: 6 (这里仅展示初始设置,action_horizon可自由设置,其较小时执行较慢但策略更稳定)
...
with torch.no_grad():
    # example inputs
    image = torch.zeros((1, obs_horizon,3,img_height,img_width))
    ...
    obs = torch.cat([image_features],dim=-1)
...
while True:
    ...
    if isUseRandomEnv:
        env.reset(isAddError=True, is_drag_mode=False, error_scale_pos=2)
    else:
        env.reset(isAddError=False, is_drag_mode=False)
    ...
    for ps in range(pred_steps):
        ...
        img_obs_tmpList.append(env.obs['img_obs'])
        
        """--------------------------------------------------"""
        """核心: get action"""
        B = 1
        # stack the last obs_horizon number of observations
        img_obs_list = np.stack([x for x in env.img_obs_list])

        # images are already normalized to [0,1]
        # print(img_obs_list.shape)
        nimage_obses = img_obs_list.transpose((0, 3, 1, 2))  # Change the channel dimension to be the first one (2,img_height,img_width,3)->(2,3,img_height,img_width)

        # device transfer
        nimage_obses = torch.from_numpy(nimage_obses).to(device, dtype=torch.float32)
        # (2,3,img_height,img_width)

        # infer action
        with torch.no_grad():
            # get image features
            image_features = ema_nets['vision_encoder'](nimage_obses.flatten(end_dim=0))
            # (2,512)
        ...
        for i in range(len(action)):
            ...
            reward_tmpList.append(env.reward)
            if env.done:
                break
