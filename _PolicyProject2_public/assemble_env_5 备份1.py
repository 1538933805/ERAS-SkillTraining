...
class AssembleEnv(object):
    def __init__(self, dt = 0., max_steps = 1000,
                 ...
                 img_obs_offsetX=[0,0], img_obs_offsetY=[25,25],
                 agent_obs_dim=12, action_dim=7, obs_horizon=4, action_horizon=8, pred_horizon=8):
        # self.task_name = "double bore shaft"
        self.task_name = "hexagonal axis"
        #|o|o|                             observations: 2
    ...
    #           initial_pos_ort=np.array([-94.62e-3, -529.85e-3, 71.5e-3, 0, -3.1415, 0])):
    def reset(self, isAddError=True, is_drag_mode=False, error_scale_pos=1, error_scale_ort=1,
              initial_pos_ort=np.array([-97.16e-3, -595.25e-3, 130e-3, 0, -3.1415, 0.5026])):
        self.initial_pos_ort = np.copy(initial_pos_ort)
        self.rob.rtde.refresh()
        "设置位置误差"
        if isAddError is True:
            error_rad = 180.0 * uniform(-1.0, 1.0) * self.DEG2RAD * error_scale_pos
            self.initial_pos_ort[0] += 0.002 * math.cos(error_rad) * error_scale_pos * uniform(0.1, 1.0)
            self.initial_pos_ort[1] += 0.002 * math.sin(error_rad) * error_scale_pos * uniform(0.1, 1.0)
    ...
    def step_by_keyboard(self, isPrintInfo=False):
        ...
        rotation_increment = 1    # 每次输入的旋转增量，单位：deg
