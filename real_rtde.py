...
class UR5_Rtde(object):
    ...
    def setPosOrt(self, pos_ort, speed=0.5, acc=0.25, threshold_time=10, asynchronous=True, 
        ...
        # self.setPosOrt_vec(pos_ort=pos_ort,speed=speed,acc=acc,threshold_time=threshold_time,asynchronous=asynchronous,
        #                    isWait=isWait, isSoft=isSoft, soft_f_threshold=soft_f_threshold, soft_return=soft_return, isPrintLog=isPrintLog)
        self.setPosOrt_rotMat(pos_ort=pos_ort,speed=speed,acc=acc,threshold_time=threshold_time,asynchronous=asynchronous,
                           isWait=isWait, isSoft=isSoft, soft_f_threshold=soft_f_threshold, soft_return=soft_return, isPrintLog=isPrintLog)
    def setPosOrt_vec(self, pos_ort, speed=0.5, acc=0.25, threshold_time=10, asynchronous=True, 
                    isWait=True, isSoft=False, soft_f_threshold=15, soft_return=0.0015, isPrintLog=False):
    ...
    "通过旋转矩阵判断相对误差"
    def setPosOrt_rotMat(self, pos_ort, speed=0.5, acc=0.25, threshold_time=10, asynchronous=True, 
                    isWait=True, isSoft=False, soft_f_threshold=15, soft_return=0.0015, isPrintLog=False):
        rot= utils.rpy_to_rotation(pos_ort[3], pos_ort[4], pos_ort[5])
        vector= cv2.Rodrigues(rot)[0].tolist()
        # 二维[[1], [2], ..., [n]]转一维[1, 2, ..., n]
        ...
        while isWait:
            ...
            cur_pos_rotvec = self.getPosRotvec()
            cur_pos_ort = self.getPosOrt()
            current_rot = utils.rpy_to_rotation(cur_pos_ort[3], cur_pos_ort[4], cur_pos_ort[5])
            diff1 = sum(abs(x-y) for x, y in zip(tcp_pos_rotvec[0:3], cur_pos_rotvec[0:3]))
            # 修正-3.14153和3.14152错误判断的情况
            # 计算旋转矩阵差值（Frobenius范数）
            diff_rot = rot - current_rot
            diff2 = np.linalg.norm(diff_rot, 'fro')
            # print(diff2)
            if (diff1 + diff2 < 0.0005) or (time.time()-start_time > threshold_time):
                # self.rtde_c.stopL(acc=0.25, asynchronous=True)
                # self.rtde_c.stopL()
        ...
        if isPrintLog: print(f'target = {pos_ort}\tnow = {self.getPosOrt()}')
