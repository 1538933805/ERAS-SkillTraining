...
class AssembleEnv(object):
    def __init__(self, dt = 0., max_steps = 1000,
                 img_obs_height = 256, img_obs_width=256, img_obs_channel=3,
                 agent_obs_dim=12, action_dim=7, obs_horizon=2, action_horizon=3, pred_horizon=12):
        #|o|o|                             observations: 2
        ...
        self.camera=realsenseD435.RealsenseD435()
        ...
        self.img_obs = np.zeros((self.img_obs_height, self.img_obs_width, self.img_obs_channel))
        self.img_obs_list = np.zeros((self.obs_horizon, self.img_obs_height, self.img_obs_width, self.img_obs_channel))
        # 整体obs部分
        self.obs = {
    ...
    def _get_obs(self, isPrintInfo=False, isShowImgs=True):
        ...
        color_image, depth_image, gray_depth_image = self.camera.get_data(sample_skip=0, isPrintShape=False,
                                                                    get_w=256, get_h=256, offset_x=0, offset_y=0)
        self.img_obs = np.copy(color_image)
        self.obs = {
            'agent_obs': self.agent_obs,
            'img_obs': self.img_obs
        }
    ...
    def reset(self, isAddError=True, is_drag_mode=False, error_scale_pos=1, error_scale_ort=1):
        # self.initial_pos_ort = np.array([-39e-3, -491e-3, 140e-3, 0, -3.1415, 0])
        # self.initial_pos_ort = np.array([-56.75e-3, -497.59e-3, 82.2e-3, 0, -3.1415, 0])
        self.initial_pos_ort = np.array([-58.1e-3, -500.15e-3, 82.2e-3, 0, -3.1415, 0])
        "设置位置误差"
        if isAddError:
    ...
    def step(self, action, isPrintInfo=False):
        ...
        if action is not None:
            # self.rob.moveIK_changePosOrt_urx(d_pos=action_real[0:3], d_ort=action_real[3:6], threshold_time=0.5, wait=True)
            self.rob.moveIK_changePosOrt(d_pos=action_real[0:3], d_ort=action_real[3:6], threshold_time=0.5, wait=True)
            # if self.action[6] > 0.5: self.rob.closeRG2()
            # else: self.rob.openRG2()
            time.sleep(self.dt)
        self.obs = self._get_obs(isPrintInfo=False)
        self.update_list_env()
        self.get_reward(isPrintInfo=True)
        self.check_episodeEnd(isPrintInfo=True)
    ...
    def step_by_drag(self, isPrintInfo=False):
        ...
        self.action[3:6] = self.normalize_angle(self.action[3:6])
    ...
    def normalize_angle(self, angle_diff):
        # 将角度差值归一化到[-pi, pi]范围内
        normalized_diff = []
        for diff in angle_diff:
            while diff > math.pi:
                diff -= 2 * math.pi
            while diff < -math.pi:
                diff += 2 * math.pi
            normalized_diff.append(diff)
        return np.array(normalized_diff)
...
if __name__ == '__main__':
    env = AssembleEnv()
    cv2.namedWindow("live", cv2.WINDOW_AUTOSIZE)
    # cv2.namedWindow("live", cv2.WINDOW_NORMAL)  # 使用WINDOW_NORMAL标志创建窗口
    ...
    while True:
        ...
        while True:
            ...
            if is_drag_mode is False:
                env.step([uniform(-10,10), uniform(-10,10), uniform(-1,1), 
                          uniform(-1,1), uniform(-1,1), uniform(-1,1)], isPrintInfo=isPrintStepInfo)
                if env.done: break
            else:
                if env.done:
